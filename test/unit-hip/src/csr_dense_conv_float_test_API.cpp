...
TEST(csr_dense_conv_float_test, func_check)
{
    ...
    control.accl_view.copy(csrValA, csr_val, num_nonzero * sizeof(float));
    ...
    control.accl_view.copy(csrValA, csr_res_val, num_nonzero * sizeof(float));
    ...
    for (int i = 0; i < num_nonzero; i++)
    {
        float diff = std::abs(csr_colInd[i] - csr_res_colInd[i]);
        EXPECT_LT(diff, 0.01);
    }

    for (int i = 0; i < num_row+1; i++)
    {
        float diff = std::abs(csr_rowPtr[i] - csr_res_rowPtr[i]);
        EXPECT_LT(diff, 0.01);
    }

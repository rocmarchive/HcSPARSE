...
TEST(gemmNnz_test, func_check)
{
    ...
    err = hipMalloc(&devA, sizeof(float)*m*k);
    err = hipMalloc(&devB, sizeof(float)*k*n);
    err = hipMalloc(&nnzPerRowColumnA, sizeof(int)*m);
    err = hipMalloc(&nnzA, sizeof(int) * 1);
    err = hipMalloc(&nnzPerRowColumnB, sizeof(int)*k);
    err = hipMalloc(&nnzB, sizeof(int) * 1);

    float *hostA = (float*)calloc (m*k, sizeof(float));
    float *hostB = (float*)calloc (k*n, sizeof(float));
    ...
    hipMemcpy(devA, hostA, m*k*sizeof(float), hipMemcpyHostToDevice);
    hipMemcpy(devB, hostB, k*n*sizeof(float), hipMemcpyHostToDevice);

    hipsparseStatus_t stat = hipsparseSnnz(handle, dir, m, k, descrA, devA, m,
                                         nnzPerRowColumnA, nnzA);
    stat = hipsparseSnnz(handle, dir, k, n, descrB, devB, k,
    ...
    err = hipMalloc(&csrRowPtrB, sizeof(int)*(k+1));
    err = hipMalloc(&csrColIndB, sizeof(int)*nnzB_h);
    err = hipMalloc(&csrRowPtrC, sizeof(int)*(m+1));
    err = hipMalloc(&nnzC, sizeof(int)*1);

    stat = hipsparseSdense2csr(handle, m, k, descrA, devA, m, nnzPerRowColumnA,
                                                 csrValA, csrRowPtrA, csrColIndA);

    stat = hipsparseSdense2csr(handle, k, n, descrB, devB, k, nnzPerRowColumnB,
                                                 csrValB, csrRowPtrB, csrColIndB);

    stat = hipsparseXcsrgemmNnz(handle, HIPSPARSE_OPERATION_NON_TRANSPOSE,
                                                  HIPSPARSE_OPERATION_NON_TRANSPOSE, m, n, k,
                                                  descrA, nnzA_h, csrRowPtrA, csrColIndA,

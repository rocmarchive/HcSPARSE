...
//    hipMemcpy(nnzPerCol, nnzPerCol_h, num_col * sizeof(int), hipMemcpyHostToDevice);
    ...
    status1 = hipsparseDnnz(handle, HIPSPARSE_DIRECTION_COLUMN, num_row, num_col, descrA, A, num_col, nnzPerCol, nnz);
    ...
    {
        double diff = std::abs(valPtr[i] - csc_res_val[i]);
        EXPECT_LT(diff, 0.01);
    }

    for (int i = 0; i < num_nonzero; i++)
    {
        double diff = std::abs(rowInd[i] - csc_res_rowInd[i]);
        EXPECT_LT(diff, 0.01);
    }

    for (int i = 0; i < num_col+1; i++)
    {
        double diff = std::abs(colPtr[i] - csc_res_colPtr[i]);
        EXPECT_LT(diff, 0.01);
    }
